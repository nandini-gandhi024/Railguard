from typing import Optional, List, Dict, Any
from fastapi import APIRouter, Depends, HTTPException, status
from pydantic import BaseModel, Field
from sqlalchemy.orm import Session

from database.database import get_db
from database.models import Track
from models.schemas import SimulationRequest, SimulationResponse
from services.risk_engine import calculate_risk
from services.weather import get_weather_context
from ai.risk_prediction import simulate_scenario

router = APIRouter(tags=["Simulation"])


class CorridorSimulationRequest(BaseModel):
    temperature_c: float = Field(38.0, example=42.0)
    monsoon_alert: bool = Field(False, example=True)
    traffic_increase_pct: float = Field(0.0, example=25.0)
    delay_days: int = Field(7, example=7)


@router.post("/simulate", response_model=SimulationResponse, summary="What-If Delayed Maintenance Simulator")
def run_simulation(req: SimulationRequest, db: Session = Depends(get_db)):
    """
    What-If Simulator Endpoint:
    Calculates current risk vs projected risk when maintenance is delayed by X days.
    """
    track = db.query(Track).filter(Track.track_id == req.track_id).first()
    if not track:
        track_dict = {
            "track_age": 14,
            "traffic_per_day": 58,
            "speed_limit": 130,
            "curve_radius": 1200.0,
            "previous_repairs": 4,
            "last_tamping_days": 210,
            "network_importance": 9
        }
    else:
        track_dict = {
            "track_age": track.track_age,
            "traffic_per_day": track.traffic_per_day * (1.0 + (req.traffic_increase_pct / 100.0)),
            "speed_limit": track.speed_limit,
            "curve_radius": track.curve_radius,
            "previous_repairs": track.previous_repairs,
            "last_tamping_days": track.last_tamping_days,
            "network_importance": track.network_importance
        }

    weather_data = {
        "temperature_c": req.temperature_c,
        "monsoon_alert": req.monsoon_alert
    }

    # Baseline risk (0 days delay)
    current_res = calculate_risk(
        track_data=track_dict,
        fault_data={"severity": 85.0, "confidence": 0.94},
        weather_data=weather_data,
        delay_days=0
    )

    # Simulated risk (delayed days)
    simulated_res = calculate_risk(
        track_data=track_dict,
        fault_data={"severity": 85.0, "confidence": 0.94},
        weather_data=weather_data,
        delay_days=req.delay_days
    )

    c_risk = current_res["risk_score"]
    p_risk = simulated_res["risk_score"]
    risk_diff = round(p_risk - c_risk, 1)

    return {
        "track_id": req.track_id,
        "current_risk": c_risk,
        "predicted_risk": p_risk,
        "risk_change": risk_diff,
        "category": simulated_res["risk_category"],
        "tsr_speed": simulated_res["tsr_speed_kmh"],
        "days_to_critical": max(0, current_res["days_to_critical"] - req.delay_days)
    }


# Endpoints for Member 5 React Frontend (RiskSimulatorLab.jsx)
@router.post("/api/simulate-risk", summary="Corridor Simulation API for React RiskSimulatorLab")
@router.post("/simulate-risk", summary="Corridor Simulation API for React RiskSimulatorLab")
def simulate_corridor_risk(req: CorridorSimulationRequest, db: Session = Depends(get_db)):
    """
    Evaluates impact of extreme heat waves, monsoon downpours, freight traffic surges,
    and delayed maintenance across the entire monitored corridor network.
    """
    tracks = db.query(Track).all()
    results = []

    for t in tracks:
        weather_data = {
            "temperature_c": req.temperature_c,
            "monsoon_alert": req.monsoon_alert
        }
        track_dict = {
            "track_id": t.track_id,
            "track_age": t.track_age,
            "traffic_per_day": t.traffic_per_day * (1.0 + (req.traffic_increase_pct / 100.0)),
            "speed_limit": t.speed_limit,
            "curve_radius": t.curve_radius,
            "previous_repairs": t.previous_repairs,
            "last_tamping_days": t.last_tamping_days,
            "network_importance": t.network_importance
        }

        sim_res = calculate_risk(
            track_data=track_dict,
            fault_data={"severity": 75.0 if t.track_id == "T041" else 45.0, "confidence": 0.92},
            weather_data=weather_data,
            delay_days=req.delay_days
        )

        results.append({
            "track_id": t.track_id,
            "section": t.section,
            "simulated_risk": sim_res["risk_score"],
            "category": sim_res["risk_category"],
            "tsr_speed": sim_res["tsr_speed_kmh"],
            "days_to_critical": sim_res["days_to_critical"]
        })

    # Sort so highest risk tracks appear first in the UI grid
    results.sort(key=lambda x: x["simulated_risk"], reverse=True)

    return {
        "parameters": {
            "temperature_c": req.temperature_c,
            "monsoon_alert": req.monsoon_alert,
            "traffic_increase_pct": req.traffic_increase_pct,
            "delay_days": req.delay_days
        },
        "results": results
    }
